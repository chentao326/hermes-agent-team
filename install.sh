...
banner() {
    echo ""
    echo -e "${CYAN}╔══════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║${NC}  ${BOLD}Hermes Agent Team${NC} — 7 人虚拟技术团队    ${CYAN}║${NC}"
    echo -e "${CYAN}║${NC}  CEO·PM·架构师·后端·前端·测试·审查       ${CYAN}║${NC}"
    echo -e "${CYAN}╚══════════════════════════════════════════╝${NC}"
    echo ""
}
...
show_help() {
    ...
    echo "  --claude    仅安装 Claude Code（7 custom agents）"
...
" 2>&1
    ...
    echo -e "  ${GREEN}→${NC}  使用: /personality pm | architect | backend | frontend | qa | reviewer | ceo"
...
install_claude() {
    ...
    if [ -d "$SCRIPT_DIR/claude-agents" ]; then
        ...
        echo -e "  ${GREEN}✓${NC}  7 个 custom agents 已安装"
        echo -e "  ${GREEN}→${NC}  使用: @pm | @architect | @backend | @frontend | @qa | @reviewer | @ceo"
...
install_codex() {
    ...
    if [ -d "$HOME/.codex/skills" ]; then
        if [ -d "$SCRIPT_DIR/codex-skills" ]; then
            ...
            echo -e "  ${GREEN}✓${NC}  Codex.app — 7 个 agent skills 已安装"
            echo -e "  ${GREEN}→${NC}  使用: /pm | /architect | /backend | /frontend | /qa | /reviewer | /ceo"
        fi
    else
    ...
    if command -v codex &>/dev/null; then
        ...
        echo -e "  ${GREEN}→${NC}  使用: codex-pm '...' | codex-architect '...' | codex-team-review '...'"
    else
...
show_summary() {
    ...
    echo -e "${BOLD}═══════════════════════════════════════════${NC}"
    echo ""
    echo "  Hermes:     /personality pm | architect | backend | frontend | qa | reviewer | ceo"
    echo "  Claude Code: @pm | @architect | @backend | @frontend | @qa | @reviewer | @ceo"
    echo "  Codex.app:  /pm | /architect | /backend | /frontend | /qa | /reviewer | /ceo"
    echo "  Codex CLI:  codex-pm '...' | codex-team-review '...'"
    echo ""
}
